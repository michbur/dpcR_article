...
Two implemented approaches, GLM and \textit{multiple testing} (\textit{MT}) were compared over 
5\e{4} simulated array dPCR experiments. Each simulation contained two runs that
had the same amount of molecules per run and both frameworks were used to compare them. The 
simulation was repeated 500 times on 10 different starting numbers of template molecules.
The GLM approach used binomial model and \textit{MT} framework employed \textit{ratio test}.

\begin{figure}[t]
\begin{center}
\includegraphics[width=9cm]{success_rate.pdf}
\end{center}
